...
async function linkView(view) {
    ...
    var linkResult = await view.server.editorComponentLinkPanel.linkComponent({
        componentName: getViewName(view.values.file),
    ...
    if (linkingError) {
        ...
        view.viewData = await view.server.editorComponentLinkPanel.getViewData({ viewName: getViewName(view.values.file) });
        PNotify.success({
            text: "View Linked.",
            type: 'notice'
        });
...
async function unlinkView(view) {
    let unlinkResult = await view.server.editorComponentLinkPanel.unlinkView({
        componentName: getViewName(view.values.file)
    });
...
class editorComponentLinkPanel extends HTMLElement {
    ...
    async setupView() {
        ...
        this.view.viewData = await this.view.server.editorComponentLinkPanel.getViewData({ viewName: getViewName(this.view.values.file) });
